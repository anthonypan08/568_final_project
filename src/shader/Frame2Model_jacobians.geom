...
void main()
{
  ...
  m[0]=0; m[1]=-1; m[2]=-1; m[3]=-1; m[4]=-1; m[5]=-1; m[6]=-1; m[7]=-1; m[8]=-1; m[9]=-1; m[10]=1; m[11]=1; m[12]=-1; m[13]=1; m[14]=-1; m[15]=1; m[16]=1; m[17]=-1; m[18]=1; m[19]=-1; m[20]=1; m[21]=-1; m[22]=-1; m[23]=-1; m[24]=-1; m[25]=-1; m[26]=-1; m[27]=-1; m[28]=-1; m[29]=-1; m[30]=1; m[31]=1; m[32]=1; m[33]=-1; m[34]=-1; m[35]=-1; m[36]=-1; m[37]=-1; m[38]=-1; m[39]=-1; m[40]=9; m[41]=-1; m[42]=-1; m[43]=-1; m[44]=9; m[45]=-1; m[46]=-1; m[47]=-1; m[48]=9; m[49]=9; m[50]=13; m[51]=13; m[52]=13; m[53]=-1; m[54]=-1; m[55]=-1; m[56]=-1; m[57]=-1; m[58]=-1; m[59]=-1; m[60]=-1; m[61]=-1; m[62]=-1; m[63]=-1; m[64]=-1; m[65]=-1; m[66]=-1; m[67]=-1; m[68]=-1; m[69]=-1; m[70]=13; m[71]=13; m[72]=9; m[73]=-1; m[74]=-1; m[75]=-1; m[76]=-1; m[77]=-1; m[78]=-1; m[79]=-1; m[80]=13; m[81]=13;
  ...
  for(int e = 0; e < entries_per_kernel; ++e)
  {
    ...
    vec2 idx_c = project2model(v_d);
    vec2 idxs[9];
    idxs[0] = vec2(idx_c.x-1, idx_c.y+1);idxs[1] = vec2(idx_c.x, idx_c.y+1);idxs[2] = vec2(idx_c.x+1, idx_c.y+1);
    idxs[3] = vec2(idx_c.x-1, idx_c.y);idxs[4] = vec2(idx_c.x, idx_c.y);idxs[5] = vec2(idx_c.x+1, idx_c.y);
    idxs[6] = vec2(idx_c.x-1, idx_c.y-1);idxs[7] = vec2(idx_c.x, idx_c.y-1);idxs[8] = vec2(idx_c.x+1, idx_c.y-1);
    ...
    for(int j=4; j<5; ++j){
      ...
      if((e_m > 1.5f) && (e_d > 1.5f))
      {
        ...
        // if(confusion_product < 0.1) inlier = false;
  ...
  if(iteration == 0){
    ...
    temp[15] = vec3(temp[15].x, temp[15].y, debug);
  }
  if(has_inlier)
  {
    for(int i = 0; i < 6; ++i)
